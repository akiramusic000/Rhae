...
public:
    ...
    funcptr_t mDtorPtr;      // 0xC
    size_t mCurElement;      // 0x10

    __partial_array_destructor(void* arr, unsigned int size, unsigned int count, funcptr_t ptr) {
        mArrayStart = arr;
        mElemSize = size;
        mArraySize = count;
        mDtorPtr = ptr;
...
void __construct_array(void* pBlock, funcptr_t ctor, funcptr_t dtor, size_t size, size_t n) {
...
void* __construct_new_array(void* block, funcptr_t ctor, funcptr_t dtor, size_t size, size_t n) {
    char* ptr = (char*)block;
...
void __destroy_arr(void* pArraySource, funcptr_t dtor, size_t size, size_t num) {
    char* cur;
...
void __destroy_new_array(void* pArraySource, funcptr_t dtor) {
    if (pArraySource != 0) {
        if (dtor != 0) {
